...
class HighestFailRateSubjectCard extends StatefulWidget {
  final ClassModel classModel;
  final ClassSemesterData semester;
  final Animation<double>? animation;

  const HighestFailRateSubjectCard({
    super.key,
  ...
  @override
  State<HighestFailRateSubjectCard> createState() => _HighestFailRateSubjectCardState();
}

class _HighestFailRateSubjectCardState extends State<HighestFailRateSubjectCard> {
  SubjectFailRateHighResponse? _subjectData;
  ...
  Future<void> _loadData() async {
    try {
      ...
      final allData = await TeacherApiService.getSubjectFailRateHigh();

      if (!mounted) return;

      // Debug: In ra thông tin để kiểm tra
      debugPrint('🔍 HighestFailRateSubjectCard - Loading data:');
      ...
      // Tìm môn học có tỷ lệ rớt cao nhất
      SubjectFailRateHighResponse? highestFailRate;
      if (filteredData.isNotEmpty) {
        highestFailRate = filteredData.reduce((a, b) =>
            a.tyLeRot > b.tyLeRot ? a : b);
      }

      setState(() {
        _subjectData = highestFailRate;
        _isLoading = false;
      });
    } catch (e) {
      if (!mounted) return;
    ...
    if (_subjectData == null) {
      return Card(
        ...
        child: Container(
          ...
          child: Center(
            child: Text(
              'Chưa có dữ liệu môn học rớt cao nhất',
              style: TextStyle(
