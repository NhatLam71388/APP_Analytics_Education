...
class GenderChart extends StatelessWidget {
  ...
  final Map<String, GenderCount>? genderCountByClass; // Dữ liệu từ API So-Luong-Sinh-Vien-Nam-Nu-Theo-Lop
                  ...
                  child: Column(
                    ...
                    children: [
                          ...
                          BarChartData(
                            ...
                            barTouchData: BarTouchData(
                              ...
                              touchTooltipData: BarTouchTooltipData(
                                getTooltipColor: (group) => Colors.pink.shade700,
                                tooltipRoundedRadius: 8,
                                tooltipPadding: const EdgeInsets.all(12),
                                tooltipMargin: 8, // Thêm margin để tooltip không bị cắt
                                getTooltipItem: (group, groupIndex, rod, rodIndex) {
                                  // Tính số lượng thực tế của từng phần (toY - fromY)
                                  final count = (rod.toY - rod.fromY).toInt();
                                  final label = rodIndex == 0 ? 'Nam' : 'Nữ';
                            ...
                            barGroups: classes.asMap().entries.map((entry) {
                              // Ưu tiên lấy từ API, fallback về dữ liệu từ ClassModel
                              final genderCount = genderCountByClass?[entry.value.tenLop];
                              final maleCount = (genderCount?.soNam ?? entry.value.maleCount).toDouble();
                              final femaleCount = (genderCount?.soNu ?? entry.value.femaleCount).toDouble();
                      ...
                      Row(
                        ...
                        children: [
                          _buildLegendItem(Colors.blue, 'Nam'),
                          const SizedBox(width: 20),
                          _buildLegendItem(Colors.pink, 'Nữ'),
                        ],
                      ),
                    ],
                  ),
